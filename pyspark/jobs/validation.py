...
if __name__ == "__main__":
    ...
    df = spark.read.json(
        "s3://ant309-test-551747521101/sample-data/aws-samples/emr-serverless/data.json")

    # Step 1 - Get daily views and uniques per repo
    gh = GitHubViews()
    daily_metrics = gh.extract_latest_daily_metrics(df)

    validation_df = daily_metrics.filter(
        daily_metrics.repo == 'aws-samples/emr-serverless')
